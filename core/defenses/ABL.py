...
class ABL(Base):
    ...
    def _train(self, dataset, schedule, loss=None, unlearning=False):
        ...
        log(msg)
        if  self.current_schedule['asr_notarget']:
                data = []
                targets = []
                for index, target in enumerate(self.poisoned_testset.targets):
                    if target != 1:
                        data.append(self.poisoned_testset.data[index])
                        targets.append(target)
                data = np.stack(data, axis=0)
                self.poisoned_testset.data = data
                self.poisoned_testset.targets = targets
        for i in range(self.current_schedule['epochs']):
            ...
            if (i + 1) % self.current_schedule['test_epoch_interval'] == 0:
                ...
                log(msg)
            if i+1 == 5 and unlearning:
                save.log_results_to_csv(acc.item(),asr.item(),'ABL.csv')
