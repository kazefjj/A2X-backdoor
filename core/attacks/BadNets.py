...
class AddTinyImageNetTrigger(AddTrigger):
    """Add watermarked trigger to TinyImageNet image.

    Args:
        pattern (None | torch.Tensor): shape (3, 64, 64) or (64, 64).
        weight (None | torch.Tensor): shape (3, 64, 64) or (64, 64).
    """
    ...
    def __call__(self, img):
        ...
        img = Image.fromarray(img.permute(1, 2, 0).numpy())
        return img
def generate_random_mapping(n, m,seed):
    if m > n:
        raise ValueError("m 不能大于 n")
    random.seed(seed)
    output_set = random.sample(range(n), m)

    while True:
        output_values = output_set.copy()
        output_values += [random.choice(output_set) for _ in range(n - m)]
        random.shuffle(output_values)
        if all(output_values[i] != i for i in range(n)):
            break 
    mapping = {i: output_values[i] for i in range(n)}
    return mapping

class CIFAR10ModifyTarget:
    def __init__(self, y_target,attack_type,attack_class,seed):
        self.y_target = y_target
        self.attack_type = attack_type
        self.num_class = 10
        if self.attack_type == 'A2O':
            self.mapping = {i: y_target for i in range(self.num_class)}
        elif self.attack_type == 'A2A':
            self.mapping = {i: (i + 1) % self.num_class for i in range(self.num_class)}
        elif self.attack_type == 'A2A+':
            with open("experiments/Mappings/mapping_CIFAR10_A210.json", "r") as f:
                mapping = json.load(f)
            self.mapping = {int(k): v for k, v in mapping.items()}
        elif self.attack_type == 'A2X':
            self.mapping = generate_random_mapping(self.num_class,attack_class,seed)
   
        elif self.attack_type == 'A2X+':
            with open(f"experiments/Mappings/mapping_CIFAR10_A2{attack_class}.json", "r") as f:
                mapping = json.load(f)
            self.mapping = {int(k): v for k, v in mapping.items()}
        elif self.attack_type == 'no':
            self.mapping ={i: i for i in range(0, self.num_class)}
        print(self.mapping)
    def __call__(self, old_target):
        return self.mapping[old_target]
    
class CIFAR100ModifyTarget:
    def __init__(self, y_target,attack_type,attack_class,seed):
        self.y_target = y_target
        self.attack_type = attack_type
        self.num_class = 100
        if self.attack_type == 'A2O':
            ...
            with open("experiments/Mappings/mapping_CIFAR100_A2100.json", "r") as f:
                mapping = json.load(f)
            self.mapping = {int(k): v for k, v in mapping.items()}
        elif self.attack_type == 'A2X':
            self.mapping = generate_random_mapping(self.num_class,attack_class,seed)
        elif self.attack_type == 'A2X+':
            with open(f"experiments/Mappings/mapping_CIFAR100_A2{attack_class}.json", "r") as f:
                mapping = json.load(f)
            self.mapping = {int(k): v for k, v in mapping.items()}
        elif self.attack_type == 'no':
            self.mapping ={i: i for i in range(0, self.num_class)}
    ...
    def __call__(self, old_target):
        return self.mapping[old_target]
class TinyImageNetModifyTarget:
    def __init__(self, y_target,attack_type,attack_class,seed):
        self.y_target = y_target
        self.attack_type = attack_type
        self.num_class = 200
        if self.attack_type == 'A2O':
            ...
            with open("experiments/Mappings/mapping_TinyImagenet_A2200.json", "r") as f:
                mapping = json.load(f)
            self.mapping = {int(k): v for k, v in mapping.items()}
        elif self.attack_type == 'A2X':
            self.mapping = generate_random_mapping(self.num_class,attack_class,seed)
        elif self.attack_type == 'A2X+':
            with open(f"experiments/Mappings/mapping_TinyImagenet_A2{attack_class}.json", "r") as f:
                mapping = json.load(f)
            self.mapping = {int(k): v for k, v in mapping.items()}
        elif self.attack_type == 'no':
            self.mapping ={i: i for i in range(0, self.num_class)}
...
class PoisonedDatasetFolder(DatasetFolder):
    def __init__(self,
        ...
        self.poisoned_target_transform.transforms.insert(poisoned_target_transform_index, CIFAR10ModifyTarget(y_target,attack_type,seed))
...
class PoisonedMNIST(MNIST):
    def __init__(self,
        ...
        if self.target_transform is None:
            ...
            self.poisoned_target_transform = copy.deepcopy(self.target_transform)
        self.poisoned_target_transform.transforms.insert(poisoned_target_transform_index, CIFAR10ModifyTarget(y_target,attack_type))
...
class PoisonedCIFAR10(CIFAR10):
    def __init__(self,
        ...
        if self.target_transform is None:
            ...
            self.poisoned_target_transform = copy.deepcopy(self.target_transform)
        self.poisoned_target_transform.transforms.insert(poisoned_target_transform_index, CIFAR10ModifyTarget(y_target,attack_type,attack_class,seed))
...
class PoisonedCIFAR100(CIFAR100):
    def __init__(self,
        ...
        if self.target_transform is None:
            ...
            self.poisoned_target_transform = copy.deepcopy(self.target_transform)
        self.poisoned_target_transform.transforms.insert(poisoned_target_transform_index, CIFAR100ModifyTarget(y_target,attack_type,attack_class,seed))
    ...
    def __getitem__(self, index):
        ...
        if index in self.poisoned_set:
            ...
            if self.target_transform is not None:
                target = self.target_transform(target)
        return img, target
class PoisonedTinyImageNet(TinyImageNet):
    def __init__(self,
                 benign_dataset,
                 y_target,
                 attack_type,
        ...
        if self.target_transform is None:
            ...
            self.poisoned_target_transform = copy.deepcopy(self.target_transform)
        self.poisoned_target_transform.transforms.insert(poisoned_target_transform_index, TinyImageNetModifyTarget(y_target,attack_type,attack_class,seed))
    ...
    def __getitem__(self, idx):
        ...
        return img, target
def CreatePoisonedDataset(benign_dataset, y_target,attack_type,attack_class, poisoned_rate, pattern, weight, poisoned_transform_index, poisoned_target_transform_index,seed):
    class_name = type(benign_dataset)
    if class_name == DatasetFolder:
        return PoisonedDatasetFolder(benign_dataset, y_target,attack_type,attack_class, poisoned_rate, pattern, weight, poisoned_transform_index, poisoned_target_transform_index,seed)
    elif class_name == MNIST:
        return PoisonedMNIST(benign_dataset, y_target, attack_type,attack_class,poisoned_rate, pattern, weight, poisoned_transform_index, poisoned_target_transform_index,seed)
    elif class_name == CIFAR10:
        return PoisonedCIFAR10(benign_dataset, y_target,attack_type,attack_class, poisoned_rate, pattern, weight, poisoned_transform_index, poisoned_target_transform_index,seed)
    elif class_name == CIFAR100:
        return PoisonedCIFAR100(benign_dataset, y_target,attack_type,attack_class, poisoned_rate, pattern, weight, poisoned_transform_index, poisoned_target_transform_index,seed)
    elif class_name == TinyImageNet:
        return PoisonedTinyImageNet(benign_dataset, y_target,attack_type,attack_class, poisoned_rate, pattern, weight, poisoned_transform_index, poisoned_target_transform_index,seed)
    else:
