...
class Base(object):
    ...
    def train(self, schedule=None):
        ...
        work_dir = osp.join(self.current_schedule['save_dir'], self.current_schedule['experiment_name'])
        ...
        self.model.train()
        if self.current_schedule['optimizer'] == 'SGD':
        ...
        elif self.current_schedule['optimizer'] == 'ADAM':
            optimizer = torch.optim.Adam(self.model.parameters(), lr=self.current_schedule['lr'], weight_decay=1e-10)
        ...
        for i in range(self.current_schedule['epochs']):
            ...
            if (i + 1) % self.current_schedule['test_epoch_interval'] == 0:
                ...
                #save.log_results_to_csv(mean_loss,0,'loss.csv')
                ...
                #Ssave.log_results_to_csv(0,mean_loss,'loss.csv')
                total_num = labels.size(0)
                prec1, prec5 = accuracy(predict_digits, labels, topk=(1, 5))
                top1_correct = int(round(prec1.item() / 100.0 * total_num))
                top5_correct = int(round(prec5.item() / 100.0 * total_num))
    ...
    def test(self, schedule=None, model=None, test_dataset=None, poisoned_test_dataset=None, test_loss=None):
        ...
        save.log_results_to_csv(acc,asr,'train.csv')
